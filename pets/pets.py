...
#Ability section process for rare.
                ...
                if ablist == []:
                    table = ["No abilities have been added. Run !pets addability <ability_name> to get started.",]
                    ablist.append(table)
                    sorted_ablist = sorted(ablist, key=itemgetter(0))                    
                    headers = ("ATTENTION!",)
    ...
    async def pets_list(self, ctx:commands.Context, *, type: str):
        ...
        if type == "pets":
            ...
            for b in beasts:
                ...
                table = [b, animal_type, description]
                all_table.append(table)
            for m in mounts:
                mount = await self.config.guild(ctx.guild).mounts.get_raw(m)
                animal_type = mount.get("type")
                description = mount.get("description")
                table = [m, animal_type, description]
                all_table.append(table)
            for r in rares:
                rare = await self.config.guild(ctx.guild).rares.get_raw(r)                
                animal_type = rare.get("type") 
        ...
        try:
            ...
            if check_ability:
                ...
                for r in rares:
                    rare = await self.config.guild(ctx.guild).rares.get_raw(r)
                    name = rare.get("name")
                    ability_name = rare.get("ability")
                    ability = ability_name.lower()
                    if ability == ability_lower:
                        return await ctx.send(f"The **{ability_name}** ability has been assigned to **{name}** and cannot be removed.\nEither change their ability or remove them first.")              
